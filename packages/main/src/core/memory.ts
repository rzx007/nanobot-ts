...
export class MemoryConsolidator {
  ...
  async consolidate(session: Session, archiveAll = false): Promise<void> {
    ...
    if (archiveAll) {
      ...
      logger.info(
        `Consolidating session memory (archive_all): ${key}, ${messages.length} messages`,
      );
    } else {
...
`;
      ...
      session.lastConsolidated = archiveAll
        ? 0
        : session.messages.length - Math.floor(this.consolidationThreshold / 2);

      logger.info(`Memory consolidation success: ${key}`);
    } catch (error) {
      logger.error(
        `Memory consolidation failed: ${session.key}, error: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
