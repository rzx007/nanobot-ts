...
function createKnightRiderTrail(options: AdvancedGradientOptions): any {
 ...
 */
export function createColors(options: KnightRiderOptions = {}): any {
  const holdStart = options.holdStart ?? 30
  const holdEnd = options.holdEnd ?? 9
  ...
  const trailOptions = {
    ...
    direction: "bidirectional" as const,
    holdFrames: { start: holdStart, end: holdEnd },
    enableFading: options.enableFading ?? true, // 设置默认值 true
    minAlpha: options.minAlpha ?? 0,           // 设置默认值 0
  }
