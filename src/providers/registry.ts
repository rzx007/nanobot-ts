...
export class LLMProvider {
  ...
  constructor(config: Config) {
    if (config.providers.openai.apiKey) {
      ...
      logger.info('OpenAI Provider 已初始化');
    }
    ...
    if (config.providers.anthropic.apiKey) {
      ...
      logger.info('Anthropic Provider 已初始化');
    }
    ...
    if (config.providers.openrouter.apiKey) {
      ...
      logger.info('OpenRouter Provider 已初始化');
    }
  }
  ...
  async chat(params: {
    ...
    try {
      ...
      logger.info(`调用 LLM: ${provider}:${modelName}`);
      ...
      const errorMsg = `LLM 调用失败: ${error instanceof Error ? error.message : String(error)}`;
